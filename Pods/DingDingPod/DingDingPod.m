...
#import "DDSettingHeader.h"

#pragma mark - Setting

CHDeclareClass(DTSettingListViewController);
CHOptimizedMethod0(self, void, DTSettingListViewController, viewDidLoad) {
    CHSuper0(DTSettingListViewController, viewDidLoad);

    DTCellItem *item = [[objc_getClass("DTCellItem") alloc] init];
    item.title = @"DingDingPod";
    item.showIndicator = YES;
    item.selectedBlock = ^(DTCellItem *item, UIView *cell) {
        [self.navigationController pushViewController:[[DDViewController alloc] init] animated:YES];
    };

    DTSectionItem *section = [objc_getClass("DTSectionItem") itemWithSectionHeader:nil sectionFooter:nil];
    section.dataSource = @[item];

    DTTableViewDataSource *tableViewDataSource = self.tableViewHandler.dataSource;
    NSMutableArray *dataSource = tableViewDataSource.tableViewDataSource.mutableCopy;
    [dataSource insertObject:section atIndex:0];
    tableViewDataSource.tableViewDataSource = dataSource;
...
CHConstructor{
    ...
    CHHook0(DTSettingListViewController, viewDidLoad);
}
